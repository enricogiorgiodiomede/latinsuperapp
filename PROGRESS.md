...
- **KNOWN, NOT FIXED (found by that linter, pre-existing, Italian only):** `js/data.js` `cleanBody` filters portrait-caption lines by the English wording `(imaginary reconstruction)` only, so the Italian `(ricostruzione immaginaria)` caption survives into two biographies - `marcus-pacuvius-and-lucius-accius` and `pomponius-bononiensis-and-quintus-novius` - and shows as garbage on their Italian author pages. Both are Archaic-era, both are paired-author entries with two portraits. One regex away from fixed; left for the user to schedule.
